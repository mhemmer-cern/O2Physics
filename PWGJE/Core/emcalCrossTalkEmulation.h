...
struct EMCCrossTalk : o2::framework::ConfigurableGroup {

  // small value to use for comarison equal to 0, std::abs(x) > epsilon
  static constexpr float Epsilon = 1e-6f;

  // default for inducedEnergyLossConstant
  static constexpr float DefaultIELC[1][4] = {{0.02f, 0.02f, 0.02f, 0.f}};

  // default for inducedEnergyLossFraction
  ...
  // default for inducedEnergyLossFractionP1
  static constexpr float DefaultIELFP1[1][4] = {{-1.1e-03, -1.1e-03, -1.1e-03, 0.f}};

  // default for inducedEnergyLossFractionWidth
  static constexpr float DefaultIELFWidth[1][4] = {{5.0e-03, 5.0e-03, 5.0e-03, 0.f}};

  // default for inducedEnergyLossMinimumFractionCentralEta, IF someone wants to try it. This is purely to document those test numbers from AliEmcalCorrectionConfiguration.yaml! Default is to not use this!
  ...
  std::string prefix = "emcCrossTalk"; // JSON group name
  ...
  std::vector<o2::emcal::Cell>* mCells;           // Pointer to the original cells of the current event
  std::vector<o2::emcal::CellLabel>* mCellLabels; // Pointer to the original cell labels of the current event
  std::vector<o2::emcal::Cell> mCellsTmp;         // Temporal vector of cells (copy)

  o2::emcal::Geometry* mGeometry; // EMCal geometry

  // histogram names
  static constexpr char NameHistBefore[] = "hCellEnergyDistBefore";
  static constexpr char NameHistAfter[] = "hCellEnergyDistAfter";

  std::shared_ptr<TH1> hCellEnergyDistBefore; // cell energy distribution, before energy smearing
  std::shared_ptr<TH1> hCellEnergyDistAfter;  // cell energy distribution, after energy smearing

  /// \brief Basic init function.
  /// \param registry histogram registry from the parent task
  void initObjects(o2::framework::HistogramRegistry& registry);
  ...
  /// \brief Fill before histogram for QA.
  void fillBeforQA();

  /// \brief Fill after histogram for QA.
  void fillAfterQA();
